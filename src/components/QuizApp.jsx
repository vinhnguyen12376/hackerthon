...
export default function QuizApp({ activeTab, setActiveTab }) {
  ...
  const startQuiz = async (skill) => {
    ...
    try {
      let queryData = [];
      let finalLimit = skill === 'random' ? 50 : 20;

      if (skill === 'random') {
        let vocabShare = 15;
        let grammarShare = 15;
        let readingShare = 10;
        let listeningShare = 10;

        const { data: { session } } = await supabase.auth.getSession();
        if (session?.user?.id) {
          const { data: lastAttempt } = await supabase.from('user_exam_attempts')
            .select('roadmap_json')
          ...
          if (lastAttempt && lastAttempt.roadmap_json && lastAttempt.roadmap_json.next_quiz_config) {
            const focusSkills = lastAttempt.roadmap_json.next_quiz_config.focus_skills || [];
            if (focusSkills.includes("grammar") || focusSkills.includes("ngữ pháp")) {
              grammarShare += 15; vocabShare -= 5; listeningShare -= 5; readingShare -= 5;
              console.log(`🪄 [AI MAGIC] Đề Daily Quiz: Nhồi thêm Ngữ pháp!`);
            }
            if (focusSkills.includes("vocabulary") || focusSkills.includes("từ vựng")) {
              vocabShare += 15; grammarShare -= 5; listeningShare -= 5; readingShare -= 5;
              console.log(`🪄 [AI MAGIC] Đề Daily Quiz: Nhồi thêm Từ vựng!`);
            }
          }
        }

        const { data: vData } = await supabase.from('exam_questions').select('*').eq('skill', 'vocabulary').limit(vocabShare);
        const { data: gData } = await supabase.from('exam_questions').select('*').eq('skill', 'grammar').limit(grammarShare);
        const { data: rData } = await supabase.from('exam_questions').select('*').eq('skill', 'reading').limit(readingShare);
        const { data: lData } = await supabase.from('exam_questions').select('*').eq('skill', 'listening').limit(listeningShare);

        queryData = [...(vData||[]), ...(gData||[]), ...(rData||[]), ...(lData||[])];
        setActiveQuizQuestions(queryData.sort(() => 0.5 - Math.random()));
      } else {
        const { data, error } = await supabase.from('exam_questions').select('*').eq('skill', skill);
        if (data) {
          setActiveQuizQuestions(data.sort(() => 0.5 - Math.random()).slice(0, finalLimit));
        } else {
